...
export default function BookingPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setSuccess(true);
      toast.success('Đặt lịch thành công!', { duration: 4000 });
      ...
      <div className="min-h-screen bg-gradient-to-br from-teal-50 to-blue-50 flex items-center justify-center p-6">
        <div className="bg-white rounded-3xl shadow-2xl p-16 text-center max-w-2xl w-full">
          ...
          <p className="text-2xl text-teal-600 font-bold">
            {form.selectedDate && new Date(form.selectedDate + 'T12:00:00').toLocaleDateString('vi-VN', {
              weekday: 'long',
              day: 'numeric',
              month: 'long',
              year: 'numeric'
          ...
          <p className="text-gray-600 mt-4">Đang chuyển về lịch của bạn...</p>
        </div>
      </div>
    );
  }
